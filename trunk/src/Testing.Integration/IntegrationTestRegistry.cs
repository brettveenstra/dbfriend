...
namespace DbFriend.Testing.Integration
{
    ...
    public class IntegrationTestRegistry : Registry
    {
        ...
        {
            ForRequestedType<IMsSqlDatabaseConnectionAdapter>().TheDefault.Is.OfConcreteType
                <MsSqlDatabaseConnectionAdapter>();

            ForRequestedType<IDbScriptFolderConfigurationSetting>().TheDefault.Is.OfConcreteType
                <DbScriptFolderConfigurationSetting>().
                WithCtorArg("outputFolder").EqualTo(
                Path.Combine(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Personal), "DbFriend"),
                             "_integrationTest"));
            ...
            if (credentialMethod == MsSqlCredentialMethod.SqlUser)
            {
                ForRequestedType<IMsSqlConnectionSettings>().TheDefault.Is.OfConcreteType<MsSqlBasicConnectionSettings>()
                    .WithCtorArg(
                    "serverInstance").EqualTo(Settings.Default.server).WithCtorArg("userName").EqualTo(
                    Settings.Default.userid).WithCtorArg(
            ...
            else
            {
                ForRequestedType<IMsSqlConnectionSettings>().TheDefault.Is.OfConcreteType
                    ...
                    "serverInstance").EqualTo(Settings.Default.server).WithCtorArg("databaseName").EqualTo(
                    Settings.Default.db);
            }
            ...
            ForRequestedType<IDatabaseScripter>().TheDefault.Is.OfConcreteType<MsSqlDatabaseScripter>();
            ...
            ForRequestedType<IConfiguredVelocityEngine>().TheDefault.Is.OfConcreteType<ConfiguredVelocityEngine>().
                WithCtorArg(
            ...
            ForRequestedType<IMsSqlDb>().TheDefaultIsConcreteType<MsSqlDb>();
            ForRequestedType<IMsSqlDbUserObjectRepository>().TheDefaultIsConcreteType<MsSqlDbUserObjectRepository>();
            ...
            ForRequestedType<IServerConnectionFactory>().TheDefaultIsConcreteType<ServerConnectionFactory>();
